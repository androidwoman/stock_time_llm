...
def convert_time_features_to_datetime(data_stamp):
    """
    Convert the normalized time features back to approximate datetime.
    This process is specific to daily frequency ('D').
    """
    batch_size, seq_len, _ = data_stamp.shape

    dates = []
    for i in range(batch_size):
        
            # Extract time components
            day_of_year = np.round((data_stamp[i, seq_len-1, 2] + 0.5) * 365 + 1).astype(int)
            year = 2022  # Fixed year assumption
            
            # Create a date using year and day_of_year
...
def test_and_save_results(args, accelerator, model, criterion, test_data, test_loader, output_file):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(test_loader)):
            ...
            # Reshape and inverse transform predictions and true values
            pred = outputs.detach().cpu().numpy().reshape(-1, 1)
            true = batch_y.detach().cpu().numpy().reshape(-1, 1)
            pred = scaler.inverse_transform(pred).flatten()
            true = scaler.inverse_transform(true).flatten()

            preds.extend(pred)
            trues.extend(true)
            print("batch_y_mark",batch_y_mark.shape)
            print(true)
            # Convert time features back to timestamps
            timestamps_batch = convert_time_features_to_datetime(batch_y_mark.cpu().numpy())
            print("------------",timestamps_batch,"\n")
            timestamps.extend(timestamps_batch)

    # Convert timestamps to a flat array
    timestamps = pd.Series(timestamps)

    # Save results
    results_df = pd.DataFrame({'timestamp': timestamps, 'true': trues, 'pred': preds})
